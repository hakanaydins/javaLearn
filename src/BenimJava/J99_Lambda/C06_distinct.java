...
// List<String> menu = new ArrayList<String>(Arrays.asList("küşleme", "küşleme", "küşleme", "soğanlı", "soğanlı", "soğanlı", "trileçe", "bicibici", "büryan", "melemen", "cacix", "kokerç", "yağlama", "güveç", "arabAşı", "tantuni"));
    ...
    //menu.stream().map(String::length()).map(t->t*t).filter(t ->t%2==0).distinct().sorted(Comparator.reversed()).forEach(t-> System.out.print(t+" "));

    menu.stream().map(String::length).filter(C01_LambdaExpression::ciftMi).map(t -> t*t).sorted().distinct().forEach(t->System.out.print(t+" "));
